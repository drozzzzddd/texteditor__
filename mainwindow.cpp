...
{
    QString title = QFileDialog::getSaveFileName(this, "Save as");
    QFile file(title);
    if(!file.open(QFile::WriteOnly | QFile::Text)){
        QMessageBox::warning(this, "Warning!", "A file with the same name already exists");
        return;
    }
...
{
    bool th;
    QFont font = QFontDialog::getFont(&th, this);
    if (th) {
